...
// --- FIX: Static middleware moved to the BOTTOM of the file ---
...
// them to access protected routes.
...
// ====== Generate OTP ======
// NOTE: This route is not prefixed with /api, so it will work with your
// existing customerSignup.html file.
...
// --- FIX: ADDED isAuthenticated MIDDLEWARE ---
app.get("/api/profile", isAuthenticated, (req, res) => {
  // if (!req.session.user) { // <-- No longer needed, middleware handles it
...
// --- FIX: ADDED isAuthenticated MIDDLEWARE ---
app.post("/api/join-saved-ride", isAuthenticated, async (req, res) => {
  try {
    // if (!req.session.user) return res.status(401).json({ success: false, message: "Not logged in" }); // <-- No longer needed
...
// --- FIX: ADDED isAuthenticated MIDDLEWARE ---
app.get("/api/recent-bookings", isAuthenticated, async (req, res) => {
  try {
    // if (!req.session.user) { // <-- No longer needed
...
// --- FIX: MOVED app.use(express.static(...)) TO THE END ---
// This serves all your HTML, CSS, and JS files from the 'public' folder.
// It MUST come *after* all your API routes (like /login, /register, /api/profile)
// to prevent the route conflict error.
